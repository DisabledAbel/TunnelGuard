...
class PerAppCountriesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        manageAppsButton = findViewById(R.id.btn_per_app_manage_apps)
        backButton = findViewById(R.id.btn_per_app_countries_back)
    ...
    private fun loadProtectedApps() {
        lifecycleScope.launch {
            ...
            backButton.nextFocusUpId = if (items.isEmpty()) manageAppsButton.id else list.id
            if (items.isEmpty()) manageAppsButton.requestFocus()
        }
    }
    ...
    private fun showCountryDialog(item: CountryItem) {
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, checked) { dialog, which ->
                ...
                val currentIndex = adapter.indexOfPackage(item.packageName)
                val currentItem = adapter.itemAt(currentIndex)
    ...
    private class CountryAdapter(
        ...
        fun indexOfPackage(packageName: String) = items.indexOfFirst { it.packageName == packageName }
        fun itemAt(index: Int) = items.getOrNull(index)
    }
