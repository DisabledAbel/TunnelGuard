...
class UpdateRepository(private val context: Context) {
    ...
    var updateChecker: UpdateChecker = GitHubUpdateCheckerImpl()
    ...
    suspend fun checkForUpdate(currentVersion: String): UpdateCheckResult = withContext(Dispatchers.IO) {
        if (isUpdateDetectedInSession()) {
            ...
            if (cachedVer != null && VersionComparator.isNewerVersion(currentVersion, cachedVer)) {
                return@withContext UpdateCheckResult.UpdateAvailable(cachedVer, getCachedApkUrl(), getCachedReleaseNotes())
            }
        }
        ...
        if (isCacheValid()) {
            ...
            if (cachedVer != null) {
                return@withContext if (VersionComparator.isNewerVersion(currentVersion, cachedVer)) {
                    ...
                    UpdateCheckResult.UpdateAvailable(cachedVer, getCachedApkUrl(), getCachedReleaseNotes())
                } else {
        ...
        val result = updateChecker.checkForLatestRelease()
        ...
        } else if (result is UpdateCheckResult.Failure) {
            if (isUpdateDetectedInSession()) {
                val cachedVer = getCachedLatestVersion()
                if (cachedVer != null && VersionComparator.isNewerVersion(currentVersion, cachedVer)) {
                    return@withContext UpdateCheckResult.UpdateAvailable(cachedVer, getCachedApkUrl(), getCachedReleaseNotes())
