...
### Compilation
To compile a debug build of TunnelGuard:
```bash
./gradlew assembleDebug
```
To run all verification checks and build:
```bash
./gradlew build
```
The generated APKs can be found under `app/build/outputs/apk/debug/`.

---

## Testing

TunnelGuard comes with a robust test suite covering configuration persistence, state machines, enums, loggers, and routing rules.

To execute the unit tests:
```bash
./gradlew test
```
Test reports are generated under `app/build/reports/tests/testDebugUnitTest/index.html`.

---

## GitHub Actions Release Automation

TunnelGuard features a secure, automated CI/CD pipeline defined in `.github/workflows/release.yml`. It uses a manual `workflow_dispatch` trigger.

### Required GitHub Secrets
Before running the workflow, the following secrets must be added to your GitHub repository:
1. `KEYSTORE_BASE64`: The Base64 encoded private signing keystore (`.jks` or `.keystore` file).
2. `KEYSTORE_PASSWORD`: The master password for your keystore.
3. `KEY_ALIAS`: The alias of the private key.
4. `KEY_PASSWORD`: The specific password of the private key.

*Note: The workflow securely reconstructs the keystore file during runtime, signs and verifies the APK, and immediately deletes all temporary credentials.*

### How to trigger a release
1. Go to the **Actions** tab of your GitHub repository.
2. Select the **TunnelGuard Release** workflow.
3. Click **Run workflow**.
4. Input the:
   * **Release Name** (e.g., `v1.0.0`)
   * **Release Description** (e.g., `Initial stable launch of TunnelGuard fail-closed TV firewall.`)
5. Click **Run workflow**.

The workflow will:
* Compile and test the application.
* Build a release APK and sign it securely.
* Verify the signature using `apksigner`.
* Generate SHA-256 checksums (`TunnelGuard-v<version>-release.apk.sha256`).
* Create a official GitHub Release.
* Upload the signed release APK, checksum files, and build metadata as release assets.
